...
- Go through every remaining devDependency in root `package.json` (eslint, `@eslint/*`,
  `@react-native/eslint-config`, `typescript`, `jest`, `@types/jest`, `prettier` + its plugins,
  `del-cli`, `turbo`) and bump to latest. `@evilmartians/lefthook` and `keep-a-changelog` are this
  repo's own (not from the rewrite) — bump those too while here.
