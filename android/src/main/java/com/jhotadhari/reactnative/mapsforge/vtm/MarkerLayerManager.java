...
public class MarkerLayerManager extends LayerManager<MarkerLayerManager.MarkerEntry> {
	...
	protected CreateResult<MarkerEntry> createEntry(
		...
		String markerUuid = UUID.randomUUID().toString();
		...
		// Track.
		MarkerEntry entry = new MarkerEntry(markerUuid, groupUuid, markerItem, positionIndex);
		allMarkers.put(markerUuid, entry);
		if (group != null) {
			group.memberMarkerUuids.add(markerUuid);
		}

		WritableMap responseData = new WritableNativeMap();
		responseData.putInt("index", itemizedLayer.getItemList().indexOf(markerItem));
		responseData.putString("uuid", markerUuid);
	...
	protected WritableMap hitTestEntry(
		...
		if (symbol.isInside(dx, dy)) {
			...
			payload.putString("uuid", entry.markerUuid);
	...
	public void updateGroup(
		...
		for (String markerUuid : group.memberMarkerUuids) {
			MarkerEntry entry = allMarkers.get(markerUuid);
	 ...
	 */
	public void setMarkerSymbol(@NonNull String markerUuid, @NonNull MarkerSymbol symbol) {
		MarkerEntry entry = allMarkers.get(markerUuid);
		if (entry != null) {
