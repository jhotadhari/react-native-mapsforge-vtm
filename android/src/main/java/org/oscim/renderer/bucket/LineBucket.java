 ...
 *
 * -----------------------------------------------------------------------------------
 * Default (unmodified) vtm LineBucket, restored for vtm 0.28.0 compatibility.
 *
 * This is the original vtm implementation with 4-short vertices (x, y, dx, dy).
 * Extensions that need 5-short vertices (e.g. path-color-ramp) shadow this class
 * with their own modified version.
 */
...
public class LineBucket extends RenderBucket {
    ...
    void addLine(float[] points, int[] index, int numPoints, boolean closed) {
        addLineImpl(points, index, numPoints, closed);
    }
    ...
    private void addLineImpl(float[] points, int[] index, int numPoints,
        ...
        for (int i = 0, pos = 0; i < n; i++) {
            ...
            addLine(vertexItems, points, ipos, length,
                    rounded, squared, closed);
        }
    }
    ...
    private void addLine(VertexData vertices, float[] points,
    ...
    public static final class Renderer {
        ...
        public static RenderBucket draw(RenderBucket b, GLViewport v,
            ...
            gl.vertexAttribPointer(s.aPos, 4, GL.SHORT, false, 8,
                    buckets.offset[LINE]);
            ...
            for (; b != null && b.type == RenderBucket.LINE; b = b.next) {
                ...
                /* draw LineLayers referenced by this outline */
