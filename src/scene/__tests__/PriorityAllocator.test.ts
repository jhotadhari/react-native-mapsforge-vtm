...
describe('PriorityAllocator', () => {
	test('initial assignment spaces entries by STEP', () => {
		...
		const changed = allocator.changedFor('frag', [
			'a',
			'b',
			'c',
		]);
	...
	test('insert at the front changes only the new entry (3000-line case)', () => {
		...
		allocator.changedFor('frag', uids);

		const changed = allocator.changedFor('frag', ['new', ...uids]);
	...
	test('removal changes nothing', () => {
		...
		expect(allocator.changedFor('frag', ['a', 'c']).size).toBe(0);
	});
	...
	test('kept entries keep priorities across mixed changes', () => {
		...
		const after = allocator.changedFor('frag', [
			'x',
			'a',
			'c',
		]);
	...
	test('gap exhaustion renumbers the whole fragment once', () => {
		...
		for (let i = 0; i < 15; i++) {
			const uid = `x${i}`;
			list.splice(1, 0, uid);
			const changed = allocator.changedFor('frag', list);
		...
		const cheap = allocator.changedFor('frag', [
			...list.slice(0, 1),
			'after-renumber',
			...list.slice(1),
		]);
	...
	test('forget clears state for destroyed fragments', () => {
		const allocator = new PriorityAllocator();
		allocator.changedFor('frag', ['a', 'b']);
		allocator.forget('frag');
		expect(allocator.changedFor('frag', ['a', 'b']).size).toBe(2);
	});

	test('fragments are isolated from each other', () => {
		const allocator = new PriorityAllocator();
		allocator.changedFor('frag1', ['a', 'b']);
		expect(allocator.changedFor('frag2', ['a', 'b']).size).toBe(2);
		expect(allocator.changedFor('frag1', ['a', 'b']).size).toBe(0);
	});
});
