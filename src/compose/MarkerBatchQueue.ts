 ...
 * MarkerBatchQueue
 *
 * Queues pending marker creation operations per nativeNodeHandle and flushes
 * them as a single batch bridge call on the microtask boundary. This collapses
 * N individual createMarker bridge calls into 1 createMarkers call.
 *
 * ## Flush timing
 *
...
function getQueue(nativeNodeHandle: number): PerHandleQueue {
	...
	if (!queue) {
		queue = { creates: [], flushScheduled: false, maxWaitTimer: null };
...
function flush(nativeNodeHandle: number): void {
	...
	LayerMarkerModule.createMarkers(batchParams)
		.then((batchResponse: MarkerBatchResponse) => {
			...
			// If the native side returned fewer results than requests
		...
		.catch((error: unknown) => {
			// Entire batch failed (map destroyed, etc.).
